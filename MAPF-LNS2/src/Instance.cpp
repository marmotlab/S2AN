...
Instance::Instance(int num_of_agents, const vector<bool>& map,
	const vector<int>& start_locations, const vector<int>& goal_locations,
	int num_of_rows, int num_of_cols, int warehouse_width):
	num_of_cols(num_of_cols), num_of_rows(num_of_rows),
	my_map(map), num_of_agents(num_of_agents), 
	start_locations(start_locations), goal_locations(goal_locations)
{
	map_size = map.size();
	map_fname = "python_map";
	agent_fname = "python_agent";
}
